...
def createModel(input_dimention, output_dimention):
    ...
    model.compile(metrics=['acc'], loss=keras.losses.mean_absolute_error, optimizer=keras.optimizers.SGD(lr=eta, momentum=momentum, nesterov=nesterov))
    return model
...
def showresult(history):
    ...
    plt.plot(epochs_array, acc_values, 'ro', label='training accuracy')
